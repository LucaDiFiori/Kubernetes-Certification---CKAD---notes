...
- **Superamento limite CPU**: Il sistema **"throttle" la CPU**, cioè  il sistema impone una limitazione sull'uso della CPU da parte di un container, impedendogli di utilizzare più risorse di quante siano specificate nel limite (**limits**) impostato.
  
	- Questo controllo viene implementato attraverso i **cgroups** (control groups) in Linux, che gestiscono e monitorano le risorse assegnate ai processi.


- **Superamento limite MEMORY**: Un container **può utilizzare più risorse di memoria rispetto a quelle imposte dal suo limits**
	- Se il pod tenta costantemente di consumare più memoria, - il nodo Kubernetes **uccide il container / pod** (evento noto come **OOMKill**, ovvero Out of Memory Kill).
	  
	- Nel log avrò un errore di tipo **OOM** (out of memory)



***
## Default Behavior
Per impostazione predefinita, **Kubernetes non ha una richiesta di CPU o limite di memoria preimpostati**.

Quindi un pod potrebbe cosnumare tutte le risorse necessarie su qualsiasi nodo togliendole agli altri
